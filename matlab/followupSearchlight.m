...
%     subject=userOptions.subjectNames{subI};
%     fprintf(['extracting fullBrain volumes for subject %d \n'],subI)
%     singleSubjectVols=fullBrainVols_Lang.(subject);
%
%     userOptions.searchlightRadius = 6;
%     mask = binaryMasks_nS.(subject).(maskName);
%     fprintf(['computing correlation maps for subject %d \n'],subI)
...
%         gotoDir(userOptions.rootPath, 'Maps');
%
%             fName= strcat(subject,  '_', maskName, '_L', modelName, '_rMap');
%        if class(fName)=='cell'
%            writeOpts.name=fName{1};
%        else
%            writeOpts.name=fName;
%        end
%         writeOpts.description=[subject '_R-Map'];
%         writeOpts.template=[userOptions.rootPath '/data/template_brain.hdr'];
%         writeMe=rs(:,:,:,modelI);
%         write_brainMap(writeMe, userOptions, writeOpts);
%     end
%     save(['Lrs_',subject,'.mat'],'rs');
%     save(['LRDMs_', subject, '.mat'], 'searchlightRDMs', '-v7.3');
...
%     subject=userOptions.subjectNames{subI};
%     fprintf(['extracting fullBrain volumes for subject %d \n'],subI)
%     singleSubjectVols=fullBrainVols_Mus.(subject);
%
%     userOptions.searchlightRadius = 6;
%     mask = binaryMasks_nS.(subject).(maskName);
%     fprintf(['computing correlation maps for subject %d \n'],subI)
...
%         gotoDir(userOptions.rootPath, 'Maps');
%
%             fName= strcat(subject,  '_', maskName, '_M', modelName, '_rMap');
%        if class(fName)=='cell'
%            writeOpts.name=fName{1};
%        else
%            writeOpts.name=fName;
...
%         write_brainMap(writeMe, userOptions, writeOpts);
%     end
%     save(['Mrs_',subject,'.mat'],'rs');
%     save(['MRDMs_', subject, '.mat'], 'searchlightRDMs', '-v7.3');
%         clear rs searchlightRDMs
%     cd(returnHere)
%
% end %subjectloop
